...
- [QSAR_package使用说明](#QSAR_package使用说明)
    - [1. 使用前准备：](#1-使用前准备)
    - [2. 提取数据/随机划分训练集测试集——必选步骤三种方式三选一](#2-提取数据/随机划分训练集测试集——必选步骤三种方式三选一)
    ...
    - [4. Pearson相关性筛选/RFE排序/数据压缩](#4-Pearson相关性筛选/RFE排序/数据压缩)
        - [4.1 Pearson相关性筛选（按训练集数据筛选）——可选步骤（一般都会用上）](#41-Pearson相关性筛选（按训练集数据筛选）——可选步骤（一般都会用上）)
        - [4.2 数据压缩——必要步骤](#42-数据压缩——必要步骤)
        - [4.3 RFE（递归消除法）排序——可选步骤](#43-RFE（递归消除法）排序——可选步骤)
    - [5. 参数寻优](#5.-参数寻优)
        - [5.1 不带描述符数量的重复网格寻优](#51-不带描述符数量的重复网格寻优)
        - [5.2 带描述符数量的重复网格寻优](#52-带描述符数量的重复网格寻优)
        - [5.3 Early_stop策略——降低过拟合程度](#53-Early_stop策略——降低过拟合程度)
