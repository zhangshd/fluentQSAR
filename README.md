...
- [QSAR_package使用说明](#QSAR_package使用说明)
    - [1. 使用前准备：](#1.-使用前准备：)
    - [2. 提取数据/随机划分训练集测试集——必选步骤，三种方式三选一](#2.-提取数据/随机划分训练集测试集——必选步骤，三种方式三选一)
        - [2.1 输入一个总描述符文件，采用随机划分的方式产生训练集和测试集](#2.1-输入一个总描述符文件，采用随机划分的方式产生训练集和测试集)
        - [2.2 根据训练集和测试集标签文件提取训练集和测试集](#2.2-根据训练集和测试集标签文件提取训练集和测试集)
        - [2.3 训练集测试集放于两个文件中](#2.3-训练集测试集放于两个文件中)
    - [3. 通过txt文件提取部分特征——可选步骤](#3.-通过txt文件提取部分特征——可选步骤)
    - [4. Pearson相关性筛选/RFE排序/数据压缩](#4.-Pearson相关性筛选/RFE排序/数据压缩)
        - [4.1 Pearson相关性筛选（按训练集数据筛选）——可选步骤（一般都会用上）](#4.1-Pearson相关性筛选（按训练集数据筛选）——可选步骤（一般都会用上）)
        - [4.2 数据压缩——必要步骤](#4.2-数据压缩——必要步骤)
        - [4.3 RFE（递归消除法）排序——可选步骤](#4.3-RFE（递归消除法）排序——可选步骤)
    - [5. 参数寻优](#5.-参数寻优)
        - [5.1 不带描述符数量的重复网格寻优](#5.1-不带描述符数量的重复网格寻优)
        - [5.2 带描述符数量的重复网格寻优](#5.2-带描述符数量的重复网格寻优)
        - [5.3 Early_stop策略——降低过拟合程度](#5.3-Early_stop策略——降低过拟合程度)
    - [6. 拟合模型/评价模型/保存结果](#6.-拟合模型/评价模型/保存结果)
